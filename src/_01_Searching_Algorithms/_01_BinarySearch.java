...
public class _01_BinarySearch {
	...
	public static int binarySearch(int[] array, int start, int end, int value) {
		...
		//   do steps 3 - 6.
			...
			if (array[end] >= array[start]) {
				status = stepsThreetoSix(array, start, end, value);
			}
	...
	static public int stepsThreetoSix(int[] array, int start, int end, int value) {
		int mid  = (start + end)/2;
		int returnValue = -1;
		if (array[mid] == value) {
			returnValue = mid;
			...
			returnValue = binarySearch(array, start, mid-1, value);
		}
		else {
			returnValue = binarySearch(array, mid + 1, end, value);
		}
		return returnValue;
	}
}	
